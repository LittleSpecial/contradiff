...
class ReplayBuffer:
    ...
    def _add_keys(self, path, skip_items = []):
        if hasattr(self, 'keys'):
            return

        self.keys = [  item for item in list(path.keys()) if item not in  skip_items  ]
    ...
    def add_path(self, path, rewrite = False,  skip_items = []):
        path_length = len(path['observations'])
        assert path_length <= self.max_path_length

        ## if first path added, set keys based on contents
        self._add_keys(path, skip_items)
        ...
        for key in self.keys:
            ...
            if rewrite and key == "rewards":
                self._dict[key][self._count, path_length:] += array[-1]

        ## penalize early termination
        if path['terminals'].any() and self.termination_penalty is not None:
            has_timeouts = bool(path.get('timeouts', np.array([False])).any())
            assert not has_timeouts, 'Penalized a timeout episode for early termination'
